		...
		<div class="max-w-lg mx-auto flex items-start justify-between gap-4">
			...
			<div class="flex flex-col items-end gap-2 pt-1">
				{#if installPrompt && !installed}
					<button
						onclick={installApp}
						class="flex items-center gap-1.5 rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-medium text-white shadow-sm hover:bg-blue-700 transition-colors"
					>
	...
	<main class="mx-auto max-w-lg space-y-4 px-4 pt-5">
		...
		{#if qrSvg}
			<div class="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
				...
				<p class="mt-1 text-xs text-slate-400">{i18n.t('qr_description')}</p>
				<div class="mt-4 flex justify-center">
					<div class="w-48 h-48 rounded-xl overflow-hidden">
						{@html qrSvg}
					</div>
				</div>
				<p class="mt-3 text-center text-[11px] text-slate-400 break-all">{typeof window !== 'undefined' ? window.location.href : ''}</p>
			</div>
		{/if}
	</main>
</div>
